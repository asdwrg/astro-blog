...
tags:
  ...
  - 协议
description:
  Hysteria2 通过魔改的 QUIC 协议，可以极大地改善网络不稳定和掉包的情况。但流量大时易被误认为 DDoS 攻击从而遭到拦截。此协议对部分移动用户而言体验不佳。
---
...
通过 ssh 连接到 vps ，输入下面的指令并运行（建议以 root 身份运行，可以用`sudo -i`来切换到 root），安装 Hysteria2 bash 脚本。
...
如果显示`curl not found`，请先安装 curl，通常安装命令为`apt-get install curl`。如果上面的 sudo 命令有同样的问题，将 curl 替换为 sudo 即可。
...
更改完成后复制到 vps 上运行即可。其中有些数据后续还会用到，最好不要删除。万一删除了也可以到`/etc/hysteria/config.yaml`查看。
...
查看最下面的日志，如果还没有显示`serve up and running`，可能是因为在申请证书，大家需要等待一下；如果成功显示，则表示服务端已配置完成。
...
此时需要重新执行 restart 指令启动服务，可以看到最后一条日志是`server up and running`，这样就成功运行了。
...
NekoBox（目前已停更）、v2rayNG 配置与电脑端类似，不过多赘述。sing-box 需要修改配置文件并导入 app 使用，参考配置文件如下，可参考[sing-box官网](https://sing-box.sagernet.org/zh/configuration/)进行个性化修改。
